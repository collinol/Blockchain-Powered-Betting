...
class Miner:
    ...
    def getDifficulty(self, header):
        ...
        #print("diffuclty as int: " + str(int(target_hexstr, 16)))
    ...
    def updateTransactionList(self):
        ...
        transactionList = self.txnMemoryPool.memoryPool[:transactionAmount]
    ...
    def calculateNonce(self):
        ...
        candidateBlock = block.Block(self.chain.blockChain[-1].blockHash, transactionList, nonce, None)

        # Find nonce that creates hash less than target hash
        while nonce < MAXNONCE:

            if transactionList < 9:
                newTransactionList = self.updateTransactionList()
                candidateBlock = block.Block(self.chain.blockChain[-1].blockHash, newTransactionList, nonce, None)
            else:
                calcMerkle = candidateBlock.blockHeader.hashMerkleRoot
                candidateBlock = block.Block(self.chain.blockChain[-1].blockHash, transactionList, nonce, calcMerkle)
